...
export default async function PropertiesPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const properties = await getProperties({ query: params.q, city: params.city, purpose: params.purpose, type: params.type, minPrice: Number(params.min) || undefined, maxPrice: Number(params.max) || undefined, bedrooms: Number(params.beds) || undefined });
